<template>
  <div class="cadet-programs">
    ...
    <div class="dynamic-page-heading">
      <video v-if="cms.cadetBackgroundVideo" :key="cms.cadetBackgroundVideo" playsinline autoplay muted loop
        :src="cms.cadetBackgroundVideo" class="bg-media"></video>
        ...
        <div class="row">
          <div class="col-12 text-center">
            <h2 class="h1 vmc-text-primary vmc-heading">
              Veritas Scholarship and Cadetship Program
            </h2>
          </div>
        </div>
      </div>
      <div v-if="!cms.cadetPrograms || cms.cadetPrograms.length === 0" class="container text-center vmc-py-big">
        <p class="text-muted">No cadet programs available at the moment.</p>
      </div>
      <!-- Loads the data from cadetPrograms instead of trainingList -->
      <div v-else>
        <div v-for="(program, index) in (cms.cadetPrograms || [])" :key="index"
          ...
          <div class="container">
            <div class="row d-flex align-items-center mt-3">
              ...
              <div :class="['col-md-4', index % 2 !== 0 ? 'order-md-1' : 'order-md-2']">
                <img v-if="program.image" :src="program.image" alt="Cadet Program" class="img-fluid mb-3 mb-md-0"
