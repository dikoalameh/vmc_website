<template>
    <div class="cms-container mb-4">
                    ...
                    <div class="w-100 mb-4" v-for="(container, index) in containers" :key="index">
                        <div class="mt-2 px-2">
                            <div class="d-flex align-items-center justify-content-between mb-2 px-2">
                                ...
                                <button type="button" class="vmc-bg-prime-6 text-white border-0 px-2 py-1 rounded"
                            ...
                            <div class="input-group mt-3 px-2">
                                <input type="text" class="input-field w-100 text-dark p-2" placeholder=""
                                    v-model="container.program">
                                <label class="floating-label mx-2">Program Title</label>
                            </div>
                            <div class="mt-3 px-2">
                                <label class="text-dark">Program Description:</label>
                                <textarea name="" id="" rows="4" class="w-100 text-dark p-1"
                                    placeholder="Description..." v-model="container.description"></textarea>
                            </div>
                            <div class="mt-3 px-2">
                                ...
                                <div
                                    ...
                                    <template v-else>
                                        ...
                                        <label :for="'upload-crewdevelopment-img-' + container.id"
        ...
        <div class="px-3">
            <button type="submit" form="crewdevelopment" class="vmc-bg-prime-4 px-3 py-1 border-0 text-white rounded">
                Save All Changes
            </button>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        removeContainer(index) {
            ...
            const input = document.getElementById(`upload-crewdevelopment-img-${container.id}`);
        ...
        resetImage(id) {
            ...
            const image = document.getElementById(`upload-crewdevelopment-img-${id}`);
            if (!image) {
            ...
            if (!image.value || image.value.trim() === "") {
                this.$refs.toast.showToast(`No attached image`, "warning")
            }
        },
        ...
        saveAllChanges(e) {
            ...
            localStorage.setItem("crewDevelopmentInput", JSON.stringify(this.containers));
            localStorage.setItem("crewDevelopmentImages", JSON.stringify(this.images));
    ...
    mounted() {
        ...
        const crewDevelopmentImage = localStorage.getItem("crewDevelopmentImages");

        if (crewDevelopmentInput) {
            this.containers = JSON.parse(crewDevelopmentInput);
            this.originalContainerInputs = JSON.parse(crewDevelopmentInput);
        }

        if (crewDevelopmentImage) {
            this.images = JSON.parse(crewDevelopmentImage);
            this.originalimages = JSON.parse(crewDevelopmentImage);
        }
    },
