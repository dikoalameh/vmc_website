<template>
    <div class="cms-container mb-4">
        ...
        <div class="d-flex align-items-center justify-content-between px-3">
            <div class="schedule-header text-dark">Schedules</div>
            <button type="button" @click="addContainer" class="vmc-bg-prime-4 border-0 text-white rounded px-2 py-1">Add
                Image</button>
        </div>
        <hr class="px-0">
        <form action="" id="schedules" @submit.prevent="saveAllChanges">
            <div class="pb-3">
                    ...
                    <div class="w-100" v-for="(container, index) in containers" :key="index">
                        <div class="px-3">
                            <div class="d-flex align-items-center justify-content-between w-100 mt-2">
                                <span class="schedule-number text-dark">Schedule Image # {{ index + 1 }}</span>
                            ...
                            <div class="mt-2">
                                <div
                                    ...
                                    <template v-else>
                                        <input type="file" :id="'upload-schedules-img-' + container.id"
                                            accept="image/png, image/jpeg, image/jpg" hidden
                                            @change="onImageChange($event, container.id)">

                                        <label :for="'upload-schedules-img-' + container.id"
        ...
        <div class="px-3">
            <button type="submit" form="schedules" class="vmc-bg-prime-4 border-0 text-white rounded px-3 py-1">
                Save All Changes
            </button>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        addContainer() {
            ...
            this.$refs.toast.showToast(`Image # ${this.containers.length} added`)
        },
        ...
        removeContainer(index) {
            ...
            const input = document.getElementById(`upload-schedules-img-${container.id}`);
            if (input) input.value = "";
            this.containers.splice(index, 1);
            this.$refs.toast.showToast(`Image # ${index + 1} removed`, "error")
        },
        ...
        resetImage(id) {
            ...
            const image = document.getElementById(`upload-schedules-img-${id}`);
            if (!image) {
        ...
        saveAllChanges(e) {
            ...
            localStorage.setItem("schedulesContainer", JSON.stringify(this.containers));
            localStorage.setItem("schedulesImages", JSON.stringify(this.images));
    ...
    mounted() {
        ...
        const schedulesContainer = localStorage.getItem("schedulesContainer");
        const schedulesImage = localStorage.getItem("schedulesImages");

        if (schedulesContainer) {
            this.containers = JSON.parse(schedulesContainer);
            this.originalContainerInputs = JSON.parse(schedulesContainer);
        }

        if (schedulesImage) {
            this.images = JSON.parse(schedulesImage);
            this.originalimages = JSON.parse(schedulesImage);
        }
    },
