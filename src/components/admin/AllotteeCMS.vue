<template>
    <div class="cms-container mb-4">
                    ...
                    <div class="w-100 box" v-for="(container, index) in containers" :key="index">
                        <div class="mb-4 px-2">
                            ...
                            <div class="input-group mt-3 px-3">
                                ...
                                <label class="floating-label mx-3">Allottees Programs</label>
                            </div>
                            <div class="year-allottees d-flex mt-3 px-3">
                                <div class="input-group mt-3">
                                    <input type="text" class="input-field w-100 text-dark p-2" placeholder=""
                                        v-model="container.year">
                                    <label class="floating-label">Year</label>
                                </div>
                                <div class="input-group mt-3">
                                    <input type="text" class="input-field w-100 text-dark p-2" placeholder=""
                                        v-model="container.activities">
                                    <label class="floating-label">Activities</label>
                                </div>
                            </div>
                            <div class="mt-3 px-3">
                                <input type="file" multiple :id="'upload-allottees-img-' + container.id"
        ...
        <div class="px-3">
            <button type="submit" form="allottees" class="vmc-bg-prime-4 px-3 py-1 border-0 text-white rounded">
                Save All Changes
            </button>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        removeContainer(index) {
            ...
            const input = document.getElementById(`upload-allottees-img-${container.id}`);
        ...
        openFileInput(id) {
            document.getElementById(`upload-allottees-img-${id}`).click();
        },
        ...
        resetImage(id) {
            ...
            const image = document.getElementById(`upload-allottees-img-${id}`);
        ...
        saveAllChanges(e) {
            ...
            localStorage.setItem("allotteesInput", JSON.stringify(this.containers));
            localStorage.setItem("allotteesImages", JSON.stringify(this.images));
    ...
    mounted() {
        ...
        const allotteesInput = localStorage.getItem("allotteesInput");
        const allotteesImage = localStorage.getItem("allotteesImages");

        if (allotteesInput) {
            this.containers = JSON.parse(allotteesInput);
            this.originalContainerInputs = JSON.parse(allotteesInput);
        }

        if (allotteesImage) {
            this.images = JSON.parse(allotteesImage);
            this.originalimages = JSON.parse(allotteesImage);
        }
    },
