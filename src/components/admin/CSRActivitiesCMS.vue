<template>
    <div class="cms-container mb-4">
                    ...
                    <div class="w-100 mb-4" v-for="(container, index) in containers" :key="index">
                        ...
                        <div class="input-group mt-3 px-3">
                            <input type="text" class="input-field w-100 text-dark p-2" placeholder=""
                                v-model="container.programs">
                            <label class="floating-label mx-3">CSR Programs</label>
                        </div>
                        <div class="d-flex mt-3 px-3" style="gap:10px;">
                            <div class="w-100">
                                <label class="text-dark">Year:</label>
                                <input type="text" class="w-100 text-dark p-1"
                                    placeholder="Year" v-model="container.year">
                            </div>
                            <div class="w-100">
                                <label class="text-dark">Type of Activities:</label>
                                <select name="" id="" class="w-100 text-dark" v-model="container.activitytype">
                                    <option value="" disabled class="text-dark">--- Choose one ---</option>
                                    <option value="Outside Veritas Activities" class="text-dark">Outside Veritas
                                        Activities</option>
                                    <option value="Inside Veritas Activities" class="text-dark">Inside Veritas
                                        Activities</option>
                                </select>
                            </div>
                        </div>
                        <div class="mt-3 px-3">
                            <input type="file" multiple :id="'upload-csr-img-' + container.id"
        ...
        <div class="px-3">
            <button type="submit" form="csr" class="vmc-bg-prime-4 text-white border-0 rounded px-3 py-1">
                Save All Changes
            </button>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        removeContainer(index) {
            ...
            const input = document.getElementById(`upload-csr-img-${container.id}`);
        ...
        openFileInput(id) {
            document.getElementById(`upload-csr-img-${id}`).click();
        },
        ...
        resetImage(id) {
            ...
            const image = document.getElementById(`upload-csr-img-${id}`);
        ...
        saveAllChanges(e) {
            ...
            localStorage.setItem("csrInput", JSON.stringify(this.containers));
            localStorage.setItem("csrImages", JSON.stringify(this.images))
    ...
    mounted() {
        ...
        const savedCSRInput = localStorage.getItem("csrInput");
        const savedCSRImage = localStorage.getItem("csrImages");

        if (savedCSRInput) {
            this.containers = JSON.parse(savedCSRInput);
            this.originalContainerInputs = JSON.parse(savedCSRInput);
        }

        if (savedCSRImage) {
            this.images = JSON.parse(savedCSRImage);
            this.originalimages = JSON.parse(savedCSRImage);
        }
    },
